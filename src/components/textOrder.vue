<template>
  <div id="textOrder">
    <h1>
      Please Enter your order in the following format (Size - Topping, Topping, Topping,...)
    </h1>
     <v-textarea
        style="width : 40vw"
          v-model="orderText"
        ...
        <modal height="auto" name="bill" id="billModal">
            ...
            <div id="items"> 
                ...
                <div class="item" v-for="order in orders" :key="order.orderText">
                    <p> {{ order.quantity + " " + order.orderText + ": $" + (order.quantity * order.price ).toFixed(2) }}</p>
                    <br>
  ...
  methods: {
    ...
    addToOrders(itemSize, itemToppings) {
      ...
      for (let i = 0; i < itemToppings.length; i++) {
        let topping = itemToppings[i]
          .split(" ")
          .join("")
          .trim();
        itemPrice += this.prices[itemSize][topping];
        if (i == itemToppings.length - 1 && itemToppings.length != 1) {
          orderText += " and " + itemToppings[i].trim();
        } else if (i == 0) {
          orderText += itemToppings[i].trim();
        } else {
          orderText += ", " + itemToppings[i].trim();
        }
      }
